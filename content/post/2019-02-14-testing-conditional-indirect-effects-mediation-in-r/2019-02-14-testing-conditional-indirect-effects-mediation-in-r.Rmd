...
## Interpretation
> Every 1&deg;F increase in room temperature was associated with an *a~1~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "a1"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "a1"], 2)`) increase in thirstiness units. Adjusting for all other predictors, every 1-unit increase in thirstiness was associated with drinking *b~1~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "b1"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "b1"], 2)`) more deciliters of water. Also, adjusting for all other predictors, there was no sufficient evidence that physically fit people drank less water than normal people, *b~2~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "b2"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "b2"], 2)`). There was also no sufficient evidence that the relation between thirstiness and water drinking depended on physical fitness group, *b~3~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "b3"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "b3"], 2)`). Increases in room temperature were associated with increases in water drinking indirectly through increases in thirstiness, but there was no sufficient evidence that this indirect effect was different between physically fit and normal people, *a~1~b~3~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "a1b3"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "a1b3"], 2)`); a bias-corrected bootstrapped confidence interval with 10,000 samples for the index of moderated mediation captured zero, 95% CI [-0.262, 0.090]. Among normal people, for every *a~1~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "a1"], 2)` unit increase in the association between room temperature and thirstiness, there was an *a~1~b~1~ + a~1~b~3~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "normie"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "normie"], 2)`) increase in deciliters of water people drank. Among physically fit people, for every *a~1~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "a1"], 2)` unit increase in the association between room temperature and thirstiness, there was an *a~1~b~1~ + a~1~b~3~* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "fitie"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "fitie"], 2)`) increase in deciliters of water people drank. Last, there was no sufficient evidence that room temperature was associated with how many deciliters of water people drank independent of its association with thirstiness, *c'* = `r round(sem.fit2@ParTable$est[sem.fit1@ParTable$label == "cp"], 2)` (*S.E.* = `r round(sem.fit2@ParTable$se[sem.fit1@ParTable$label == "cp"], 2)`).
