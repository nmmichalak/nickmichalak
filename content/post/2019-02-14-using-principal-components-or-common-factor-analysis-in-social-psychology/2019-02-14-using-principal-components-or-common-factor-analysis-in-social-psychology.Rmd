...
> Here is one way to represent the distance axioms:  
> 1. $$d_{ij} > d_{ii} = 0,  for  i  \neq j.$$  
> 2. $$d_{ij} = d_ji.$$  
> 3. $$d_{ij} \leq d_{ik} + d_{kj}.$$  
> Here is one way to represent the similarity axioms:  
> 1. $$s_{ii} > s_{ij}.$$  
> 2. $$s_{ij} = s_ji.$$  
> 3. a large  $$s_{ij}  implies  d_{ik} \approx d_{kj}.$$
