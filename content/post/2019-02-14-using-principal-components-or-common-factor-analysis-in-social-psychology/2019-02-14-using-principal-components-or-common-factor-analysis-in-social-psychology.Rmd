...
# In closing, use the Non-Metric Multidimensional Scaling procedure on Big Five Inventory items
...
#### Interpretation
> To be rigorous, I used a non-metric version of Multidimensional Scaling that uses a "non-monotonic" function (i.e., ordinal, non-linear) to find a solution. I only asked for 2 dimensions, so the solution doesn't quite capture what we might expect: The items from the unique traits didn't cluster together perfectly in this 2-dimensional space. But the items do seem to "cluster" together a little like you'd expect. Perhaps other technqiues could better capture how these items cluster together in traits (e.g., k-means clustering). Maybe that's for another post. In any case, these procedures are pretty impressive!
